...
export class LeadDeduplicationManager {
  ...
  // Remove duplicates from existing campaign leads
  static async removeDuplicatesFromCampaign(
    campaignId: string,
    userId: string,
    supabase: any
  ): Promise<{
    ...
    try {
      ...
      const { data: leads, error: fetchError } = await supabase
        ...
        .eq('user_id', userId)
        .order('created_at', { ascending: true }); // Keep oldest leads
      ...
      leads.forEach(lead => {
        ...
        if (normalizedPhone) {
          if (phoneMap.has(normalizedPhone)) {
            ...
            phoneMap.set(normalizedPhone, lead.id);
          }
        }
        ...
        if (normalizedEmail) {
          if (emailMap.has(normalizedEmail)) {
            ...
            emailMap.set(normalizedEmail, lead.id);
          }
        }
